...
(define (evaluate nums ops)
  (let loop ((result (car nums))
    ...
    (if (null? rest-ops)
        result
        (loop (case (car rest-ops)
                ((+) (+ result (car rest-nums)))
                ((*) (* result (car rest-nums)))
                ((||) (concatenate result (car rest-nums))))
              (cdr rest-nums)
              (cdr rest-ops)))))

(define (generate-ops len)
  (let ((cons-plus  (lambda (r) (cons '+ r)))
        (cons-mult  (lambda (r) (cons '* r)))
        (cons-cat   (lambda (r) (cons '|| r))))
    (if (= len 0)
        '(())
        (append (map cons-plus (generate-ops (- len 1)))
                (map cons-mult (generate-ops (- len 1)))
                (map cons-cat  (generate-ops (- len 1)))))))
